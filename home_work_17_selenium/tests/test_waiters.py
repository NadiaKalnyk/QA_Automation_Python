...
class TestWaiters:
    ...
    @pytest.mark.skip  # декоратор, щоб тест не запускався
    ...
    def test_connection_enable(self):
        self.driver.get("https://demoqa.com/dynamic-properties")
        enabled_button_loc = (By.ID, "enableAfter")
        enabled_button: WebElement = self.driver.find_element(*enabled_button_loc)
        WebDriverWait(self.driver, timeout=5).until(ec.element_to_be_clickable(enabled_button))
